...
const Header = () => {
  ...
  return (
    <section className='sticky top-0 left-0 right-0 z-50 bg-white'>
        <header className="container mx-zuto px-5 flex justify-between py-4 items-center">
            <div>
                <img className='w-16' src={images.Logo} alt="logo" />
            </div>
            <div className='z-50 lg:hidden'>
                {navIsVisible ? <AiOutlineClose className = "w-6 h-6" onClick={navVisibility} /> : <AiOutlineMenu className = "w-6 h-6" onClick={navVisibility} /> }
            </div>
            <div className={`${navIsVisible ? "right-0" : "-right-full"} transition-all duration-300 mt-[56px] lg:mt-0 bg-dark-hard lg:bg-transparent z-[49] flex flex-col w-full lg:w-auto justify-center lg:justify-end lg:flex-row fixed top-0 bottom-0 lg:static  flex gap-x-9 items-center`}>
                <ul className='text-white items-center lg:text-dark-soft flex flex-col lg:flex-row py-10 gap-y-5 gap-x-3 font-semibold'>
